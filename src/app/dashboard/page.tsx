...
const texts = {
  ...
  ...Object.keys(featuresData).reduce((acc, key) => {
    ...
    acc[`${key}_title` as keyof typeof acc] = featuresData[featureKey].title;
    acc[`${key}_description` as keyof typeof acc] = featuresData[featureKey].description;
    acc[`${key}_cta` as keyof typeof acc] = featuresData[featureKey].cta;
    return acc;
...
export default function DashboardPage() {
  ...
  const features = Object.keys(featuresData).map(key => {
    ...
    return {
      ...
      title: t(`${key}_title` as keyof typeof texts),
      description: t(`${key}_description` as keyof typeof texts),
      cta: t(`${key}_cta` as keyof typeof texts),
    }
  });
